...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c, final MPI mpi) throws MPIException {
    	...
    	int curRank = mpi.MPI_Comm_rank(mpi.MPI_COMM_WORLD);
		int ranksNum = mpi.MPI_Comm_size(mpi.MPI_COMM_WORLD);
		int rowsNum = c.getNRows();
		int chunkSize = (rowsNum + ranksNum - 1) / ranksNum;
		int startRow = curRank * chunkSize;
		int endRow = (curRank + 1) * chunkSize;
		if (endRow > rowsNum) endRow = rowsNum;
        ...
        if (curRank == 0) {
			MPI.MPI_Request[] mpiRequests = new MPI.MPI_Request[ranksNum - 1];
			for (int i = 1; i < ranksNum; i++) {
				int rankStartRow = i * chunkSize;
				int rankEndRow = (i + 1) * chunkSize;
				if (rankEndRow > rowsNum) rankEndRow = rowsNum;

				int rowOffset = rankStartRow * c.getNCols();
				int elementsAmount = (rankEndRow - rankStartRow) * c.getNCols();

				mpiRequests[i - 1] = mpi.MPI_Irecv(c.getValues(), rowOffset, elementsAmount, i, i, mpi.MPI_COMM_WORLD);
			}

			mpi.MPI_Waitall(mpiRequests);
		} else {
        	mpi.MPI_Send(c.getValues(), startRow * c.getNCols(), (endRow - startRow) * c.getNCols(),
				0, curRank, mpi.MPI_COMM_WORLD);
		}
    }
}
